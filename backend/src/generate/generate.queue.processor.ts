...
export class GenerateQueueProcessor {
  ...
  async processGenerationJob(job: Job<GenerationJobData>) {
    ...
    this.logger.log(
      `🔄 [${type.toUpperCase()}] Processing job ${jobId} (queued: ${queueWaitTime}ms)`,
    );
    ...
    try {
      ...
      this.logger.debug(
        `[${type.toUpperCase()}] DB update: ${Date.now() - dbUpdateStart}ms`,
      );
      ...
      if (type === "script") {
        this.logger.log(
          `[SCRIPT] 📝 Generating script from story (${story?.length || 0} chars)`,
        );
        ...
        this.logger.log(
          `[IMAGES] 🖼️  Generating ${job.data.style || "default"} style images`,
        );
        ...
        this.logger.log(
          `[VIDEO] 🎬 Assembling video (${job.data.fps || 30}fps, ${job.data.bitrate || "2000k"})`,
        );
      ...
      if (type === "video") {
        ...
        if (videoUrl) {
          try {
            await this.projectService.finalizeFromVideoJob(
              jobId,
              userId,
              videoUrl,
            );
      ...
      this.logger.error(
        `❌ [${type.toUpperCase()}] Job ${jobId} FAILED after ${totalTimeMs}ms: ${errorMsg}`,
      );
