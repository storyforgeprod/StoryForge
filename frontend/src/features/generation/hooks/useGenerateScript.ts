...
export type UseGenerateScriptReturn = {
    ...
    generate: (title: string, story: string, targetDuration?: number, targetScenes?: number, tone?: string, language?: string) => void;
    reset: () => void;
};
...
export function useGenerateScript(): UseGenerateScriptReturn {
    ...
    const generate = useCallback(
        async (title: string, story: string, targetDuration?: number, targetScenes?: number, tone?: string, language?: string) => {
            setState({ phase: 'submitting' });
            try {
                const { jobId } = await postGenerateScript({ title, story, targetDuration, targetScenes, tone, language });
